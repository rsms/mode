...
var _baseDir = path.dirname(path.dirname(path.dirname(
...
Module.prototype.isActive = function(callback){
  var self = this;
  fs.realpath(this.activePath, function(err, rpath) {
    callback(null, !err && rpath === self.productPath);
  });
}

Module.prototype.isInstalled = function(callback){
  var self = this;
...
Module.prototype._fetchGit = function(options, callback) {
  ...
  fs.stat(this.installDir, function(err, stats) {
    if (!err && stats && stats.isDirectory()) {
      ...
      // todo: if cone fails due to git: being blocked, try with http: if the
      //       repoURI refers to github (since GH do support git over http).
...
Module.prototype._installPrep = function(options, callback) {
  if (typeof options === 'function') {
    callback = options;
    options = undefined;
  }
...
Module.prototype.upgrade = function(options, callback) {
  var jobs = this._installPrep(options, callback);
  var self = this;
  // require the module to be installed when upgrading
  jobs.unshift(function(cl){
    this.isInstalled(function(err, installed){
      cl(installed ? null : new Error(self+' is not installed'));
    });
  });
  // (do not activate after an upgrade)
  jobs.start();
}

Module.prototype.install = function(options, callback) {
  var jobs = this._installPrep(options, callback);
  // activate module after a successful installation or upgrade
  jobs.push(function(cl){ this.activate(options, cl); });
  jobs.start();
}

Module.isManaged = function(dir, callback) {
  function loadmod(dir) {
    var relpath, abspath, module, 
      v = dir.substr(exports.installedDir.length).replace(/^\/+/,'').split('/');
    relpath = v[0]+'/'+v[1];
    module = new Module(fnnoext(relpath));
    abspath = path.join(exports.indexDir, relpath)+'.js';
    module.loadFromIndex(abspath, function(err) {
      if (err) return callback(err);
      callback(null, module);
    });
  }
  if (dir.indexOf(exports.installedDir) === 0) {
    loadmod(dir);
  }
  else {
    fs.realpath(dir, function(err, rdir) {
      if (rdir.indexOf(exports.installedDir) === 0)
        loadmod(rdir);
      else
        callback();
    });
...
Module.find = function(query, options, callback) {
  ...
  // find matches
  var modules = [], self = this, dir,
      findAux = options.findInstalled || options.findActive;
  if (options.findInstalled)
    dir = exports.installedDir;
  else if (options.findActive)
    dir = exports.activeDir;
  else
    dir = exports.indexDir;
  var findOpt = {
    dirnames: dir,
    filter: query,
    maxDepth: options.findActive ? 2 : 3,
  };
  var cl = fs.find(findOpt, function(err){
    modules.sort(function(a, b){ return (a.id < b.id) ? -1 : 1; });
    callback(err, modules);
  });
  if (options.findInstalled) {
    cl.addListener('directory', function(relpath, abspath){
      if (relpath.charAt(0) === '.' || relpath.indexOf('/') === -1 ||
        relpath.indexOf('/.') !== -1)
      {
        return;
      }
      cl.open();
      Module.isManaged(abspath, function(err, module) {
        if (module)
          modules.push(module);
        cl.close();
      });
    ...
    function checkActive(relpath, abspath){
      if (relpath.charAt(0) === '.')
        return;
      cl.open();
      Module.isManaged(abspath, function(err, module) {
    ...
    cl.addListener('directory', checkActive);
    cl.addListener('file', checkActive);
  }
  else {
    cl.addListener('file', function(relpath, abspath){
      if (path.extname(relpath) !== '.js' || relpath.indexOf('/') === -1)
        return;
