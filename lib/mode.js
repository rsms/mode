...
Module.prototype.__defineGetter__('productPath', function(){
  ...
  if (this.config.product) p.push(this.config.product);
  return p.join('/');
});
...
Module.prototype.configure = function(options, callback) {
  ...
  var findJobs = new CallQueue(this, false, function(){
    // any jobs?
...
Module.prototype.activate = function(options, callback) {
  ...
  var mklink = function(){
    ...
    fs.mkdirs(path.dirname(dstpath), function(err, pathsCreated){
      ...
      var link = path.relativize(path.dirname(dstpath), self.productPath);
      // path.relativize might break links when forwarded?
      //var link = self.productPath;
