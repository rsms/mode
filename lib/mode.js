...
Module.prototype._clean = function(options, callback) {
  var self = this;
...
Module.prototype._checkoutGit = function(options, callback) {
  ...
  git.exec(args, execopt, function(err, so, se){
    if (err && err.message && err.message.indexOf('HEAD is now at ') === 0)
      err = 0;
...
Module.prototype._fetchGit = function(options, callback) {
  ...
  fs.stat(options.checkoutDir, function(err, stats) {
    if (!err && stats && stats.isDirectory()) {
      ...
      git.exec(args, execopt, function(err, so, se){
        self.emit('did-fetch');
        if (self.info.rev || self.info.repoBranch)
...
Module.prototype._configure = function(options, callback) {
...
Module.prototype._build = function(options, callback) {
  // todo: build
  // todo: if (needed) ..
  return callback();
...
Module.prototype._install = function(options, callback) {
  this.emit('will-install');
  var self = this;
  if (!options.installLinks || options.installLinks.length === 0) {
    var e = new Error('Nothing to install (no installLinks configured)');
    if (callback)
      callback(e);
    return e;
  }
...
Module.prototype.install = function(options, callback) {
  ...
  */
  // todo: check if already cloned/in cache
  // todo: check if already installed
  if (!this.info.repo)
    throw new Error('unable to install module '+this+' -- no repo configured');
  ...
  // queue jobs
  var jobs = new CallQueue(this, false, function(err){ callback(err); });
  
  if (options.force)
    jobs.push(function(cl){ this._clean(options, cl); });
  // fetch // todo: add support for other kinds of repositories
  jobs.push(function(cl){ this._fetchGit(options, cl); });
  jobs.push(function(cl){ this._configure(options, cl); });
  jobs.push(function(cl){ this._build(options, cl); });
  jobs.push(function(cl){ this._install(options, cl); });
