...
Module.prototype.__defineGetter__('checkoutDir', function(){
  return path.join(exports.installedDir, this.id);
});
...
Module.prototype.uninstall = function(options, callback) {
  var self = this;
  fs.stat(this.config.checkoutDir, function(notfound) {
    if (notfound) {
      if (callback) callback(null, false);
      return;
    }
    self.emit('will-uninstall');
    sh.rm(self.config.checkoutDir, 'rf', function(err){
...
Module.prototype._checkoutGit = function(options, callback) {
  ...
  var self = this;
  var execopt = {worktree:this.config.checkoutDir, outbuf:false, errfwd:true};
  var args = ['checkout', '--force'];
  if (!options.verbose) args.push('--quiet');
  args.push(self.config.repoRevision || 'master');
...
Module.prototype._fetchGit = function(options, callback) {
  ...
  var self = this;
  fs.stat(this.config.checkoutDir, function(err, stats) {
    if (!err && stats && stats.isDirectory()) {
      ...
      args = fixargs(args);
      args.push('origin');
      args.push(self.config.repoBranch || 'master');
      var execopt = {worktree:self.config.checkoutDir, outbuf:false, 
                     errfwd:true};
      git.exec(args, execopt, function(err, so, se){
        self.emit('did-fetch');
      ...
      else if (self.config.repoBranch)
        args = args.concat(['--branch', self.config.repoBranch]);
      args.push(self.config.checkoutDir);
...
Module.prototype.configure = function(options, callback) {
  ...
  // assure this.config.products
  if (!Array.isArray(this.config.products)||this.config.products.length===0) {
    if (Array.isArray(this.info.products))
      this.config.products = this.info.products;
    else
      this.config.products = [this.config.checkoutDir];
  }
  
  // trim "/"
  var v = this.config.products;
  for (var i=0;i<v.length;i++) { v[i] = v[i].replace(/^[\/]+|[\/]+$/, ''); }
...
Module.prototype.activate = function(options, callback) {
  // todo: respect options.force
  // todo: only if needed
  this.emit('will-activate');
  var self = this;
  if (!this.config.products || this.config.products.length === 0) {
    var e = new Error('Nothing to activate (no products configured)');
    if (callback)
      callback(e);
    return e;
  }
  sys.error('TODO: Module.activate');
  callback();
  /*
  var args = ['ln', '-sf'];
...
Module.prototype.install = function(options, callback) {
  ...
  */
  ...
  if (!this.config.checkoutDir || options.checkoutDir)
    this.config.checkoutDir = options.checkoutDir || this.checkoutDir;
  
  if (!this.config.repoURI || options.repoURI)
    this.config.repoURI = options.repoURI || this.info.repo;
  
  if (!this.config.repoBranch || options.repoBranch)
    this.config.repoBranch = options.repoBranch || this.info.repoBranch;
