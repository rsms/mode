...
Module.prototype.__defineGetter__('activeId', function(){
  var tag = this.config.repoBranch;
...
Module.prototype._fetchGit = function(options, callback) {
  ...
  fs.stat(this.config.checkoutDir, function(err, stats) {
    if (!err && stats && stats.isDirectory()) {
      ...
      args = fixargs(args);
      args.push('origin');
      args.push(self.config.repoBranch);
      ...
      else if (self.config.repoBranch && self.config.repoBranch !== 'master')
  ...
  fs.lstat(dstpath, function(err, stats){
    if (err) {
      if (err.errno === process.ENOENT) {
        // not active
        self.emit('will-activate'); self.emit('did-activate');
...
Module.prototype._prepareConfig = function(options) {
  ...
  // setup this.config
  if (!conf.checkoutDir || options.checkoutDir)
    conf.checkoutDir = options.checkoutDir || this.checkoutDir;
  
  if (!conf.product)
    conf.product = info.product || conf.checkoutDir;
  
  if (!conf.repoURI || options.repoURI)
    conf.repoURI = options.repoURI || info.repo;
  
  if (!conf.repoBranch || options.repoBranch)
    conf.repoBranch = options.repoBranch || info.repoBranch || 'master';
